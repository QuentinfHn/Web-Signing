...
class SignageCacheDB {
    ...
    async deleteScreensAndStates(screenIds: string[]): Promise<void> {
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                screenIds.forEach((screenId) => {
                    screensStore.delete(screenId);
                    statesStore.delete(screenId);
                });
            });
        } catch (error) {
            console.error('Error deleting screens and states:', error);
            throw error;
        }
    }
}
